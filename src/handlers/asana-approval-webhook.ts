...
import { asanaWebhookPayloadSchema, type ApprovalLookup } from "../contracts/email-workflow.js";
...
import { isTaskTokenReplayError, StepFunctionsService } from "../services/step-functions.js";
...
export async function handler(
  ...
  const hookSecret = event.headers["x-hook-secret"] ?? event.headers["X-Hook-Secret"];
  if (hookSecret) {
    return {
      statusCode: 200,
      headers: {
  ...
  try {
    ...
    for (const webhookEvent of payload.events) {
      ...
      if (
        ...
        results.push({ taskGid, status: "ignored", reason: "not_task_completion" });
        continue;
      }

      const approval = await tokenStore.get(taskGid);
      if (!approval) {
        results.push({ taskGid, status: "ignored", reason: "no_pending_approval" });
        continue;
      }

      const task = await asana.getTask(taskGid);
      if (!task.completed) {
        results.push({ taskGid, requestId: approval.requestId, status: "ignored", reason: "not_completed" });
        continue;
      }
...
function isApprover(actor: ApprovalActor | undefined | null, approval: ApprovalLookup): boolean {
  if (!actor) {
    return false;
  }
...
async function approve(
  ...
  try {
    ...
    return { taskGid: approval.taskGid, requestId: approval.requestId, status: "approved" };
  } catch (error) {
    if (isTaskTokenReplayError(error)) {
      await tokenStore.delete(approval.taskGid);
      return { taskGid: approval.taskGid, requestId: approval.requestId, status: "replay_ignored" };
    }
    throw error;
  }
}
...
async function rejectNonApprover(
  ...
  try {
    await stepFunctions.sendTaskFailure(approval.taskToken, "NonApproverCompletion", {
      requestId: approval.requestId,
      taskGid: approval.taskGid,
      actor: actor
        ? {
